...
function anagrams(stringA, stringB) {
  ...
  const stringAcharObj = {};

  for (let i = 0; i < stringA.length; i++) {
    stringAcharObj[stringA[i]] = stringAcharObj[stringA[i]] + 1 || 1;
  }

  for (let i = 0; i < stringB.length; i++) {
    if (!stringAcharObj[stringB[i]]) {
      return false;
    } else {
      stringAcharObj[stringB[i]]--;
    }
  }
...
// O(N * log N) = worse than 1
function anagrams(stringA, stringB) {
  stringA = stringA.toLowerCase().replace(/[\W_]+/g, "");
  stringB = stringB.toLowerCase().replace(/[\W_]+/g, "");

  let sanitizedStringA = stringA.split("").sort().join("");
  let sanitizedStringB = stringB.split("").sort().join("");
